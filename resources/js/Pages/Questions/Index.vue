      ...
      <table class="w-full whitespace-nowrap">
        <thead>
          <tr class="text-left font-bold">
            ...
            <th class="pb-4 pt-6 px-6" colspan="2">Topic</th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="question in questions.data" :key="question.id" class="hover:bg-gray-100 focus-within:bg-gray-100">
            <td class="border-t">
              <Link class="flex items-center px-6 py-4 focus:text-indigo-500" :href="`/questions/${question.id}/edit`">
                {{ question.text }}
                <icon v-if="question.deleted_at" name="trash" class="flex-shrink-0 ml-2 w-3 h-3 fill-gray-400" />
              </Link>
            </td>
            <td class="border-t">
  ...
  watch: {
    form: {
      ...
      handler: throttle(function () {
        this.$inertia.get('/questions', pickBy(this.form), { preserveState: true })
      }, 150),
    },
  },
  methods: {
